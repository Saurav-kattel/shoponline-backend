const User = require("../modules/users2");
...
router.post(
  ...
  async (req, res) => {
    let success=false;
    // req is the requested or send by user and res is what user recieves
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      return res.status(400).json({ success,errors: errors.array() });
    }
    try {
      // try checking if there is an email which already exits from user module
      let user = await User.findOne({ email: req.body.email });
      if (user) {
        return res.status(400).json({success, error: "user already exits" });
      }
      // creates a random text of length 10
      const salt = bcrypt.genSaltSync(10);
    //creates hash i.e text using the salt and password for security reason
      var hash =bcrypt.hashSync(req.body.password, salt);
      // create user using  name,email and password and password will be stored in hash form
      ...
      success=true;
// to create jwt which is used to verify user 
